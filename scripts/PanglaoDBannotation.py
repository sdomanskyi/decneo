...
def extractPanglaoDBannotation(dirName, df_cell_type_annotations, SRA, SRS, saveFile = True):
    ...
    if not os.path.exists(os.path.join(dirName, 'PanglaoDBannotation')): 
        os.makedirs(os.path.join(dirName, 'PanglaoDBannotation'))
    ...
    df_panglao_annotation = df_panglao_annotation['celltype'].astype(str)
    df_panglao_annotation.name = 'cell'
    df_panglao_annotation = pd.concat([df_panglao_annotation], keys=['%s_%s' % (SRA, SRS)], names=['batch'], axis=0)
